...
(define (generate p q e)
  (list (list (m p q) e) (list (m p q) (d e (L p q)) ) ) )
...
(define (encrypt plaintext key)
  (helper1 (string->list plaintext) key))

(define (helper1 chars key)
  (cond ((null? chars) '())
        (else (cons (modulo (expt (char->integer (car chars)) (car (cdr key))) (car key)) (helper1 (cdr chars) key)))))
...
(define (decrypt ciphertext key)
  (list->string (helper2 ciphertext key)))

(define (helper2 ciphertext key)
  (cond ((null? ciphertext) '())
        (else (cons (integer->char (modulo (expt (car ciphertext) (car (cdr key))) (car key))) (helper2 (cdr ciphertext) key)))))
