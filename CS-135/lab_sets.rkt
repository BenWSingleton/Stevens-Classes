...
(define (union LA LB)
  (cond ((null? LA) (make-set LB))
        (else (union (cdr LA) (cons (car LA) LB)))))
...
(define (set-difference LA LB)
  (cond ((null? LA) '())
        ((element? (car LA) LB) (set-difference (cdr LA) LB))
        (else (make-set (cons (car LA) (set-difference (cdr LA) LB))))))
...
(define (cardinality L)
  (cond ((null? L) 0)
        ((element? (car L) (cdr L)) (cardinality (make-set L)))
        (else (+ 1 (cardinality (cdr L))))))
...
(define (superset? SA SB)
  (cond ((null? SB) #t)
        ((element? (car SB) SA) (superset? SA (cdr SB)))
        (else #f)))
