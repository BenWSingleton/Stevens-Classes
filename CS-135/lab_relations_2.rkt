...
(define (compose RO RI)
  (cond ((null? RI) '())
        (else (union (helper1 RO (car RI)) (compose RO (cdr RI))))))

(define (helper1 RO I)
  (cond ((null? RO) '())
        ((equal? (car (car RO)) (car (cdr I))) (make-set (cons (list (car I) (car (cdr (car RO)))) (helper1 (cdr RO) I))))
        (else (helper1 (cdr RO) I))))
...
(define (helper2 R k P)
  (cond ((equal? k 0) '())
        ((equal? k 1) P)
        (else (helper2 R (- k 1) (compose R P)))))
...
(define (transitive-closure R)
  (helper3 R 1 (cardinality R) R))

;Works for case 2 and 3 but not 1
(define (helper3 R Num Count Original)
  (cond ((equal? Num (+ Count 1)) R)
        (else (helper3 (union R (power Original Num)) (+ Num 1) Count Original))))
...
(define (EQ-relation? R n)
  (and (transitive? R) (and (symmetric? R) (reflexive? R n))))
