...
(define (euclid-gcd a b)
  (cond ((and (= a 0) (= b 0)) 0)
        ...
        ((= (modulo a b) 0) b)
        ((= (modulo b a) 0) a)
        ((< a b) (euclid-gcd a (- b a)))
        (else (euclid-gcd (- a b) b))))
...
(define (CRT-exists? cong-sys)
  (cond ((null? (cdr cong-sys)) #t)
        (else (helper1 (cdr cong-sys) (car (cdr (car cong-sys))) cong-sys ))))  

(define (helper1 cong-sys prev original)
  (cond ((null? cong-sys) (CRT-exists? (cdr original)))
        ((not (= (euclid-gcd (car (cdr (car cong-sys))) prev) 1)) #f)
        (else (helper1 (cdr cong-sys) prev original))))
...
(define (m cong-sys)
  (cond ((null? cong-sys) 1)
        (else (* (car (cdr (car cong-sys))) (m (cdr cong-sys))))))
...
(define (CRT-helper cong-sys m)
  (cond ((null? cong-sys) 0)
        (else (+ (* (ai cong-sys) (* (Mi cong-sys m) (yi cong-sys m))) (CRT-helper (cdr cong-sys) m)))))

(define (ai cong-sys)
  (car (car cong-sys)))
        
(define (Mi cong-sys m)
  (/ m (car (cdr (car cong-sys)))))

(define (yi cong-sys m) ;;mul-inv of Mi (mod bi)
  (mul-inv (Mi cong-sys m) (car (cdr (car cong-sys)))))
...
(define (CRT cong-sys)
  (cond ((CRT-exists? cong-sys) (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys)))
        (else -1)))
