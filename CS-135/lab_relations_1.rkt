...
(define (reflexive-closure R n)
  (cond ((not (reflexive? R n)) (make-set (append (id n) R)))
        (else R)))
...
(define (symmetric-closure R)
  (cond ((not (symmetric? R)) (make-set (append R (inverse R))))
        (else (car (list R)))))
...
(define (relates-to v R)
  (cond ((null? R) '())
        ((equal? v (car (car R))) (append (cdr (car R)) (relates-to v (cdr R))))
