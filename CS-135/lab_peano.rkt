...
(define (helper1 a b O)
  (cond ((pz? b) a)
        (else (helper1 (p+ a O) (pred b) O))))
...
(define (p^ a b)
  (cond ((pz? b) 1)
        ((pz? a) 0)
        (else (helper2 a (pred b) a))))
 
(define (helper2 a b O)
  (cond ((pz? b) a)
        (else (helper2 (p* a O) (pred b) O))))
...
;; Type signature: (p% peano peano) -> peano
(define (p% a b)
  (cond ((pz? b) "<modulo error>")
        ((pz? a) 0)
        (else (helper3 a b))))

(define (helper3 a b)
  (cond ((equal? (checker a b) #f) a)
        (else (helper3 (p- a b) b))))

;; returns #f if a minus b becomes negative, true otherwise
(define (checker a b)
  (cond ((p>= a b) #t)
        (else #f)))
...
(define (p-prime? p)
  (cond ((p= z p) #f)
        ((p= (succ z) p) #f)
        (else (helper4 p (pred p)))))

(define (helper4 p c)
  (cond ((p= c z) #t)
        ((p= c (succ z)) (helper4 p (pred c)))
        ((equal? (divisible p c) #t) #f)
        (else (helper4 p (pred c)))))
