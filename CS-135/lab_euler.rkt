...
(define (degree G v)
  (helper1 (apply append G) v 0))

(define (helper1 G v count)
  (cond ((null? G) count)
        ((= v (car G)) (helper1 (cdr G) v (+ 1 count)))
        (else (helper1 (cdr G) v count))))
...
(define (all-even? G)
  (helper2 G (apply append G)))

(define (helper2 G g)
  (cond ((null? g) #t)
        ((not (even? (degree G (car g)))) #f)
        (else (helper2 G (cdr g)))))
...
(define (helper3 G v)
  (cond ((null? G) '())
        ((= (car (car G)) v) (cons (car (cdr (car G))) (adjacent (cdr G) v)))
        ((= (car (cdr (car G))) v) (cons (car (car G)) (adjacent (cdr G) v)))
...
(define (connected? G)
  (cond ((null? G) #t)
        (else (helper4 G (apply append G) (usort (dfs G '() (car (car G))))))))


(define (helper4 G g ref)
  (cond ((null? g) #t)
        ((equal? ref (usort (dfs G '() (car g)))) (helper4 G (cdr g) ref))
        (else #f)))
...
(define (eulerian? G)
  (cond ((and (equal? #t (connected? G)) (equal? #t (all-even? G))) #t) 
        (else #f)))