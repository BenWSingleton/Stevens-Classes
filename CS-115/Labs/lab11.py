...
def insertV2(L, i):
    ...
    save=L[i]
    insert=search(L,i,save)
    for n in range(i,insert,-1):
        L[n]=L[n-1]
    L[insert]=save
    return L
...
def letterCounts(fname):
    ...
    #print(allWords)
    for let in allWords:
        if letter(let)=="non-letter":
            if "non-letter" in count:
                count['non-letter']+=1
            else:
                count['non-letter']=1
        if letter(let)==let.lower():
            if let in count:
                count[let]+=1
            else:
                count[let]=1
