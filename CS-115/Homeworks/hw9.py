...
def wordScoreLoop(S, scorelist):
    ...
    for n in range(len(S)):
        total=total+letterScore(S[n], scrabbleScores)
    return total
...
def wordsWithScore(dct, scores):
    ...
    with its value. For example, wordsWithScore(aDictionary, scrabbleScores)
...
def wordsWithScoreLoop(dct, scores):
    ...
    result=[]
    for n in dct:
        result=result+([[n, wordScore(n, scores)]])
    return result
