...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
    	const K& key = key_value.first; //what do we do with this???
        Node<K, V> *x, *y;
        if (it != end()) {
            x = it.node_ptr;
            y = x->parent;
        ...
        // TODO
        //Still needs to throw tree_exception
        //Are we setting up the new node correctly?
        if (find(key)!=end()) {
        	std::stringstream oss;
        	oss << key;
        	std::string message = "Attempt to insert duplicate key '" + oss.str() + "'.";
        	throw tree_exception(message);
        	//How do we set up the exception correctly?
        }
    ...
    //Functions we added to test null_count, sum_levels, and sum_null_levels
    size_t null_count() {
    	return null_count();
    }
    ...
    size_t sum_null_levels() {
    	return sum_null_levels();
    }
...
private:
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	return std::max(left_height + right_height + 3, std::max(left_diam, right_diam));
